...
const DeviceLogs: React.FC<{ id: string }> = (props) => {
  ...
  const getList = () => {
    ...
    getDeviceLog(props.id, page).then((res) => {
      if (res.data && res.data.list && res.data.list.length) {
        res.data.list.sort((a: DEVICE.deviceLogs, b: DEVICE.deviceLogs) => {
          return b.number - a.number;
        });
        const list = res.data.list.map((e: DEVICE.deviceLogs) => {
          const {
            year,
            month,
            date,
